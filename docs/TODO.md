...
- **Mobil — floating status bar, volně přesouvatelná po obrazovce (Frank 2026-08-17):** "chci, aby při zobrazení aplikace na telefonu bylo možné mít floating status bar a volně ho přesouvat po obrazovce." Nejasné, který prvek je míněný — appka už MÁ draggable "Live HUD" (`#live-strip`/`.live-hud`, `_liveHudDrag` mechanismus, feel-fader.html ~5519+, pozice se ukládá do `ff_live_hud_pos`), ale kompaktní connection-status pill v horní liště (`.h-status`/`#h-status`, feel-fader.html ~1075-1107, uvnitř `.top-sticky`) draggable NENÍ — možná jde o tenhle. Upřesnit s Frankem, jestli myslí (a) rozšířit existující Live HUD drag i na `.h-status`, nebo (b) něco jiného, než se začne implementovat.
