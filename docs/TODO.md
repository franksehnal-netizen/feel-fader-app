...
- **Articulation tagy (Roller order, UACC pilulky) vypadají "cheap" kvůli gradientu (Frank 2026-08-18, screenshot):** "tyto tlačítka vypadají strašně cheap s tím gradientem." Jde o `.uacc-tag` (feel-fader.html ~595, `background:var(--control-glass-bg)`) — `--control-glass-bg` je `linear-gradient(145deg,rgba(255,255,255,.72),rgba(174,174,178,.34))` (feel-fader.html ~61, dark varianta ~131). **Pozor:** tenhle token je sdílený napříč appkou (`.ui-glass`, `.send-btn.idle`, `.roller-mode-row`, `.stepper input`, `.ks-stepval` a další) — rozhodnout, jestli řešit jen `.uacc-tag` samostatně (vlastní pozadí bez gradientu), nebo přehodnotit sdílený `--control-glass-bg` token šířeji (dopad na víc míst appky).
