...
- [ ] **Step 8: Update the existing help-deep-links probe — two of its three checks click buttons this task removes**

`scratch/help-deep-links-probe.mjs` is an existing, already-registered regression probe from a prior batch. It directly queries and clicks `.tx[aria-label^="Help"]` inside `.section-head` (the fader "?" button) and `.tx[aria-label="Help: Keyboard (HID)"]` inside `.info-row-action` (the HID row "?" button) — **both buttons this task deletes**. Left unmodified, `document.querySelector(...).click()` on the now-`null` result would throw and crash this probe. `openHelpAt()` itself is untouched by this task, but two of its three exercised entry points no longer exist.

Replace the full contents of `scratch/help-deep-links-probe.mjs` with:
...
Expected: all `PASS` (the surviving `hidEnableNotice` check still opens the Help panel correctly; the two new assertions confirm the removed buttons are actually gone, closing the loop with `hover-tip-probe.mjs`'s own — narrower — check of the same fact).
...
```bash
git add feel-fader.html scratch/hover-tip-probe.mjs scratch/help-deep-links-probe.mjs scratch/run-all-probes.mjs
