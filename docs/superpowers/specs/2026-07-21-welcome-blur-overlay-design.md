...
8. Toggle "skrýt controller" (`applyStageCollapse`/`applySendAnchorDock`) už nepotřebuje odložené časování na handoff moment (ten přestává existovat) — obojí se spustí rovnou při loadu. Třída bugu "zaseknuté tlačítko na welcome screenu" (2026-07-21) tím zmizí strukturálně, ne jen záplatou.
